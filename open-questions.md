...
- Does the fit-history approach actually predict well against real users, or does accuracy collapse outside the now-12 seeded brands (only 1 of which — Westside — is verified)? First real test: 1 of 1 testers said yes to the prediction, but that test only covered same-measurement-type brands — still unknown whether it holds for a cross-type case.
